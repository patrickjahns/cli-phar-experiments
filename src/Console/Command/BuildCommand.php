...
class BuildCommand extends Command
{
	...
	private function mirror($appPath, $tmpPath, $configuration) {
		$finder = new Finder();
	...
	private function generateArchive($tmpPath, $appPath) {
		$finder = new Finder();
		$finder->in($tmpPath);
	...
	private function generateSignature($tmpPath, $privateKeyPath, $keyBundlePath) {
		$rsa = new RSA();
	...
	private function executePreCommands(SymfonyStyle $io, $appPath, $commands) {
		foreach($commands as $command) {
			...
			$cmd->setTimeout(600);
			$cmd->run(function ($type, $buffer) use ($io) {
				if (Process::ERR === $type) {
